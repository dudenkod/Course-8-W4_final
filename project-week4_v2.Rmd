...
Hcorr <- findCorrelation(cor(train_caret[, -c(2,5,59)]), cutoff=0.5)
Hcorr
...
str(test_caret)
test_caret_final = test_caret[, -c(2,5,59)][,-Hcorr]
test_caret_final = cbind(test_caret_final, test_caret[, c(2,5,59)])

table(train_caret_final$classe)

reduced_Ti = createDataPartition(y=train_caret_final$classe, p=0.1, list = FALSE)

table(train_caret_final[reduced_Ti,]$classe)

inTrain = createDataPartition(y=train_caret_final[reduced_Ti,]$classe, p=0.75, list = FALSE)

Train_FINAL = train_caret_final[reduced_Ti,][inTrain,]
Test_FINAL = train_caret_final[reduced_Ti,][-inTrain,]

table(Train_FINAL$classe)
table(Test_FINAL$classe)

dim(Train_FINAL)

control <- rfeControl(functions=rfFuncs, method="cv", number=10)
# run the RFE algorithm
results <- rfe(Train_FINAL[,-c(27)], Train_FINAL[,27], sizes=c(1:10), rfeControl=control)
...
str(Train_FINAL)
model_RF = train(classe~., data=Train_FINAL, method="rf", preProcess="range", trControl=control, tuneGrid = expand.grid(.mtry = c(2:6)),  n.tree=200, maxit=2000)
...
PREDICTION = predict(model_RF, newdata=Test_FINAL[,-c(27)])
confusionMatrix(PREDICTION,Test_FINAL[,27])

dim(test_caret)
str(test_caret)
PREDICTION_COURSE = predict(model_RF, newdata=test_caret)
summary(PREDICTION_COURSE)

model_RF_noX = train(classe~., data=Train_FINAL[,-c(1)], method="rf", preProcess="range", trControl=control, tuneGrid = expand.grid(.mtry = c(2:6)),  n.tree=200, maxit=2000)


qplot(train_caret$num_window, colour=train_caret$classe)

library("ggvis")
train_caret %>% ggvis(~num_window, ~classe,  fill = ~classe) %>% layer_points()
